...
export const AmountInput = ({ amount, isValid, onAmountChange }: Props) => {
    ...
    const handleAmountChange = useCallback((amountStr: string) => {
...
const Input = styled.TextInput<{ isValid: boolean }>`
    ...
    border-bottom-color: ${({ isValid, theme }) => isValid ? theme.palette.content.secondary : theme.palette.indicator.error};
    padding-bottom: 14px;
`